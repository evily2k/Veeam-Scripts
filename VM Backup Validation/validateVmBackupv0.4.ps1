...
# Get today's date and format ita and save as a string
$todaysDate = (get-date).ToString("yyyyMMdd")
# VBV report directory
$reportDir = "C:\temp\VeeamFLR\Report"
# Report file name
$VBVReportName = "Veeam Validator Results.txt"
# VBV report file full path
$VBVReport = "$($reportDir)\$($VBVReportName)"
# VBV report header to seperate each job VBV results
...
{	
	...
	foreach ($vm in $vmRestore)
	{
		...
		If ($validatorResults -eq 'Success'){
			# Success log entry
			[string]$VBVlog = "[$(Get-Date -format 'u')] [SUCCESS] [$vmName] Backup file successfully validated"
		...
		Else{
			# Failure log entry
			[string]$VBVlog = "[$(Get-Date -format 'u')] [FAILURE] [$vmName] $volName - Backup file validation failed"
			# Write results to event log
			Write-host $VBVlog
