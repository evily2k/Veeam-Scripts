...
{	
	...
	foreach ($vm in $vmRestore)
	{
		...
		If ($validatorResults -eq 'Success'){
			...
			# Success log entry
			$flrLog = "[$(Get-Date -format 'u')] [SUCCESS] [$vmName] Backup files successfully validated"
		...
		Else{
			...
			$eventType = "Error"
			# Failure log entry
			$flrLog = "[$(Get-Date -format 'u')] [FAILURE] [$vmName] $volName - Backup file validation failed"
			# Write results to event log
